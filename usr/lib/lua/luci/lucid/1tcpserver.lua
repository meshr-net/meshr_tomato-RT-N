...
function prepare_daemon(config, server)
   ...
   local sopts = {reuseaddr = 1, rcvtimeo = 10}
   ...
   local handler, err = config.slave and config.slave.module.factory(publisher, config) or nil
   if not handler then
      for _, s in ipairs(sockets) do
         s:close()
      end
...
function prepare_socket(family, host, port, opts, backlog)
   nixio.syslog("info", "Preparing socket for host " .. ( host or "" ) .. " port " .. port)
