...
function call(...)
  ...
  if hostos:sub(1,3) == 'win' then
     local prefix = arg[1]:sub(1,1) == '/' and rootfs or ''
     local cmd = 'bash -c "export meshr=\"' .. rootfs .. '\"; export PATH=\"$PATH\"; cd ' .. rootfs .. '/etc/init.d; ' .. prefix:gsub('\\','/') .. ... .. '"'
...
function sysinfo()
   ...
   local model =
      luci.util.pcdata(fs.readfile(rootfs .. "/tmp/sysinfo/model")) or
...
function uniqueid(bytes)
   local rand = fs.readfile(hostos:sub(1,3) == 'win' and rootfs .. '/proc/urandom' or  "/dev/urandom", bytes) --TODO
...
function net.routes(callback)
   ...
   for line in io.lines((hostos:sub(1,3) == 'win' and rootfs or '') .. "/proc/net/route") do
...
function init.index(name)
   if fs.access(init.dir..name) then
      return call("env -i bash -c 'source %s%s enabled; exit ${START:-255}' >/dev/null"
         %{ init.dir, name })
   end
end
